...
Participants' names coded for anonymity, and their responses edited for clarity and brevity.
    ...
    From the interviews, we observed that all the participants believe that climate change is urgent and needs to be planned for. \pr{1} mentions that although the effects of climate change will be seen on the tech field, it is not isolated to the tech field and should be considered as a global problem.
    ...
    \begin{quoting}
        \textit{``I think the poorer will be more affected by it, but the richer can reduce the impacts of climate change for themselves.''} (\pr{1})
    \end{quoting}
    ...
    \begin{quoting}
        \textit{``I don't personally fear for myself or about whether where I live down there is going to catch on fire.''} (\pr{2})
    \end{quoting}
    ...
    \begin{quoting}
        \textit{``I've read suggestions of using your current appliances till the end of their lifespan, and to then recycle them, avoiding byproducts that would harm the environment. For instance, I plan on buying an electric car or hybrid in the future, but I'll drive my current car until it is no longer operable.''} (\pr{4})
    \end{quoting}
...
\subsection{Is computing affected by climate change?}
    ...
    \begin{quoting}
        \textit{
        ``As we know, the effects of climate change can be recorded in databases. If we want to get more useful information, we cannot ignore the powerful tools of the \textsc{ai}.''}(\pr{5})
    \end{quoting}
    ...
    \begin{quoting}
        \textit{
        ``When I was an undergrad in architecture, the topic of climate change was very much in my everyday language, but since taking the \textsc{hci} path, it kind of lost it a bit...''        }(\pr{2})
    \end{quoting}
    ...
    \begin{quoting}
        \textit{
        ``Some large-scale machine learning use a lot of electricity, and their long-term goals are not aligned with climate trends.'' (\pr{5})
        }
    \end{quoting}
...
\subsection{Is climate change affecting computing?}
    ...
    Many participants believe the current situation has raised people's awareness about climate change, and that climate change-related topics research will become more prominent:
    
    \begin{quoting}
        \textit{``As climate change continues to be a pressing issue, and so does our use of technology, there are many opportunities for people in \textsc{hci} to integrate that with climate change somehow.''} (\pr{2})
    \end{quoting}
    
    \begin{quoting}
        \textit{``In terms of meteorological studies, there would be more modeling for weather changes or stuff like that.''} (\pr{1})
    \end{quoting}
    
    \begin{quoting}
        \textit{``There would be more games geared towards the climate change issue, with preventing it as their main topic.''} (\pr{4})
    \end{quoting}
    
    \begin{quoting}
        \textit{``I think that they (games) would let people be more conscious about how they affect the climate.''} (\pr{4})
    \end{quoting}
    
    % One participant was uncertain since they were just into the front-end development field. They still needed to learn rules or knowledge to adapt to new technologies. So, currently,  it is hard for her to predict whether these technologies will change in the future due to climate issues. 
